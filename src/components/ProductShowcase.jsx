...
const ProductPanel = ({ panel, index }) => {
  ...
  useEffect(() => {
    ...
    const obs = new IntersectionObserver(
      ([e]) => { if (e.isIntersecting) { el.classList.add('panel-in'); obs.disconnect(); } },
      { threshold: 0.1 }
    );
    obs.observe(el);
  ...
  return (
    ...
    >
      ...
      `} style={{ color: panel.accent }}>
        {panel.id === 'pure-aura' ? 'NATURAL' : panel.id === 'calm-pure' ? 'BALANCE' : 'RITUAL'}
      </span>
      ...
      `} style={{ background: panel.accent }} />
        <span className={`
        panel-kicker
      `} style={{ color: t.sub }}>{panel.kicker}</span>
        <h2 className={`
        panel-name
      `}>{panel.name}</h2>
        <span className={`
        panel-tagline
      `} style={{ color: t.sub }}>{panel.tagline}</span>
      ...
      `}
          ...
          onMouseEnter={e => { e.currentTarget.style.background = t.ctaHover; e.currentTarget.style.color = t.ctaHoverText; }}
...
const ProductShowcase = () => (
  <section className={`
      ...
      `} id="products">
    {PANELS.map((panel, i) => (
      <ProductPanel key={panel.id} panel={panel} index={i} />
    ))}
  </section>
);
